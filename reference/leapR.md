...
relative to a random distribution. Note that currently `method` Defaults
to `ks` for the KS test, but can be changed to the `ztest` to run the z
statistic. `primary_columns` only accepts a single column for this
method.  
