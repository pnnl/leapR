...
#'
#' @param geneset is a list of four vectors, gene names, gene descriptions, gene
#' sizes and a matrix of genes. It represents .gmt format pathway files.
#' @param enrichment_method is a character string specifying the method of
...
#' @param eset is a `SummarizedExperiment` object containing expression data,
#' with features as rows and \emph{n} sample/conditions as columns.
#' @param assay_name is the assay to be analyzed within the `eset`.
...
leapR <- function(geneset, enrichment_method, eset, assay_name, ...){
  ...
  #there are three required elements; an enrichment method, a geneset, and a
  #summarized experiment
  stopifnot(enrichment_method %in% c("correlation_enrichment",
                                     "correlation_comparison",
                                     "enrichment_in_order",
                                     "enrichment_comparison",
                                     "enrichment_in_pathway",
                                     "enrichment_in_sets"),
            is(geneset,'geneset_data'),
...
.enrichment_wrapper <- function(geneset,enrichment_method, eset, assay_name,
  ...
  if (enrichment_method == "enrichment_in_order") {
    ...
    stopifnot(!is.null(primary_columns), length(primary_columns) == 1)
    ...
    stopifnot(is.element(primary_columns, all_names))
    ...
    if (!is.null(id_column)) {
      if (!is.element(id_column,
                      colnames(SummarizedExperiment::rowData(eset)))) {
          stop("'id_column' must be a column name of 'rowData(eset)'")}
    }

    if (is.null(targets)) {
      if (!is.null(id_column)) {
        targets <- unique(SummarizedExperiment::rowData(eset)[targ_ind,
                                                              id_column])
      }else{
    ...
    if (length(targets) == 0)
    {
        stop("please adjust 'threshold' argument, \
           there are no target genes captured by current threshold value")}

      result <- enrichment_in_groups(geneset = geneset, targets = targets,
                                     mapping_column = id_column,
                                     background = eset, method = "fishers",
                                     minsize = minsize)

  }

  else if (enrichment_method == "enrichment_in_pathway") {
      result <- enrichment_in_abundance(geneset = geneset, eset = eset,
                                        assay_name = assay_name,
                                        mapping_column = id_column,
                                     abundance_column = primary_columns,
                                     sample_comparison = NULL, fdr = fdr,
                                     min_p_threshold = min_p_threshold,
                                     sample_n = sample_n)
      # min_p_threshold is a parameter that will change the output -
      result <- as.data.frame(result)
  }

  else if (enrichment_method == "enrichment_comparison") {
      stopifnot(!is.null(secondary_columns), !is.null(primary_columns))
      result <- enrichment_in_abundance(geneset = geneset, eset = eset,
                                      assay_name = assay_name,
                                      mapping_column = id_column,
                                      abundance_column = primary_columns,
                                      sample_comparison = secondary_columns,
                                      fdr = fdr,
                                      min_p_threshold = min_p_threshold,
                                      sample_n = sample_n)
    ...
    temp_result <- correlation_enrichment(geneset = geneset,
                                            eset = eset,
                                            assay_name = assay_name,
                                            mapping_column = id_column)
    ...
    if (is.null(id_column)) {id_column <- NA}
    temp_result <- correlation_comparison_enrichment(geneset = geneset,
                                                     eset = eset,
                                                     assay_name = assay_name,
                                                     set1 = primary_columns,
                                                     set2 = secondary_columns,
                                               mapping_column = id_column)
