...
calcTTest <- function(eset, assay_name, group1, group2){
  ...
  res <- t(vapply(rownames(exprdata), function(r) {
    if (length(which(!is.na(exprdata[r,shortlist, drop = FALSE]))) > 2 && length(which(!is.na(exprdata[r,longlist, drop = FALSE]))) > 1) {
      res <- t.test(exprdata[r,shortlist, drop = FALSE], exprdata[r,longlist, drop = FALSE])
