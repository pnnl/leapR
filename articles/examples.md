...
`SummarizedExperiment` object and computes the t-test between them. The
results are then stored in the `rowData` of the same object, so that
they can be used for enrichment later on.
...
![](examples_files/figure-html/figure_2-1.png)

Figure 3. An application of the KSEA-like approach in leapR as applied
to our example data. In this example we are looking for known substrate
sets of kinases (from Phosphosite Plus) that are enriched in the short
...
![](examples_files/figure-html/figure_3-2.png)
