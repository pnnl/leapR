...
###using enrichment_wrapper function
protdata.enrichment.fishers = leapR(geneset=ncipid,
                                   enrichment_method="enrichment_by_fishers",
                                   datamatrix=protdata, threshold=.5,
                                   primary_columns="TCGA-13-1484")

knitr::kable(protdata.enrichment.fishers[order(protdata.enrichment.fishers[,"pvalue"]),][1:10,])
...
for (i in 1:5) {
  ...
  knitr::kable(results[order(results[,"pvalue"]),][1:10,])
...
protdata.enrichment.ks = leapR(geneset=ncipid, "enrichment_by_ks", 
                                            datamatrix=protdata, 
                                            primary_columns=shortlist[1])
knitr::kable(protdata.enrichment.ks[order(protdata.enrichment.ks[,"pvalue"]),][1:10,])
```
