...
knitr::kable(protdata.enrichment.svl[order(protdata.enrichment.svl[,6]),][1:10,])
...
knitr::kable(protdata.enrichment.svl.ovo[order(protdata.enrichment.svl.ovo[,6]),][1:10,])
```
...
###using enrichment_wrapper function
prodata.enrichment.fishers = leapR(geneset=ncipid,
                                   enrichment_method="enrichment_by_fishers",
                                   datamatrix=protdata, threshold=.5,
                                   primary_columns="TCGA-13-1484")

knitr::kable(protdata.enrichment.fishers[order(protdata.enrichment.fishers[,6]),][1:10,])
...
protdata.enrichment.fishers.module_1 = leapR(geneset=ncipid, enrichment_method="enrichment_by_fishers",
...
for (i in 1:5) {
  ...
  results = leapR(geneset=ncipid, enrichment_method="enrichment_by_fishers",
                  background=names(modules), targets=names(modules[which(modules==i)]))
  
  knitr::kable(results[order(results[,6]),][1:10,])
...
###using enrichment_wrapper function
protdata.enrichment.ks = leapR(geneset=ncipid, "enrichment_by_ks", 
                                            datamatrix=protdata, 
                                            primary_columns=shortlist[1])
knitr::kable(protdata.enrichment.ks[order(protdata.enrichment.ks[,6]),][1:10])
```
...
###using enrichment_wrapper function
protdata.enrichment.correlation = leapRr(geneset=ncipid, 
                                        enrichment_method = "correlation_enrichment",
                                        datamatrix=protdata)

knitr::kable(head(protdata.enrichment.correlation[order(protdata.enrichment.correlation[,6]),]))
protdata.enrichment.correlation.short = leapR(geneset=ncipid, 
                                              enrichment_method = "correlation_enrichment", 
                                              datamatrix=protdata[,shortlist])
knitr::kable(head(protdata.enrichment.correlation.short[order(protdata.enrichment.correlation.short[,6]),]))

protdata.enrichment.correlation.long = leapR(geneset=ncipid, 
                                            enrichment_method = "correlation_enrichment",
                                            datamatrix=protdata[,longlist])
knitr::kable(head(protdata.enrichment.correlation.long[order(protdata.enrichment.correlation.long[,6]),]))
```
