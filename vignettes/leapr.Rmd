...
protdata.enrichment.sets.modules = cluster_enrichment(geneset=ncipid, clusters=clusters, background=names(modules))
...
``` {r figure_1, echo=TRUE, warning=FALSE}
>>>>>>> 26b93e692f35544a3ad29cfe4ca89cb4b5fa9210

#load the single omic and multi-omic pathway databases
data("krbpaths")
data("mo_krbpaths")

# enrichment in transcriptional data
transdata.enrichment.svl = leapR(geneset=krbpaths, 
                                enrichment_method='enrichment_comparison',
                                datamatrix=transdata, primary_columns=shortlist,
                                secondary_columns=longlist)

# enrichment in proteomics data
# this is the same code used above, just repeated here for clarity
protdata.enrichment.svl = leapR(geneset=krbpaths, 
                                enrichment_method='enrichment_comparison',
                                datamatrix=protdata, primary_columns=shortlist,
                                secondary_columns=longlist)

# enrichment in phosphoproteomics data
phosphodata.enrichment.svl = leapR(geneset=krbpaths,
                                   enrichment_method='enrichment_comparison',
...
pvaluesmat = cbind(transdata.enrichment.svl[,"pvalue"], protdata.enrichment.svl[,"pvalue"], phosphodata.enrichment.svl[,"pvalue"], combodata.enrichment.svl[,"pvalue"])
rownames(pvaluesmat) = rownames(phosphodata.enrichment.svl)
colnames(pvaluesmat) = c("TXN", "PROT", "PHOSPHO", "COMBO")

zscoresmat = cbind(transdata.enrichment.svl[,"zscore"], protdata.enrichment.svl[,"zscore"], phosphodata.enrichment.svl[,"zscore"], combodata.enrichment.svl[,"zscore"])
rownames(zscoresmat) = rownames(phosphodata.enrichment.svl)
colnames(zscoresmat) = c("TXN", "PROT", "PHOSPHO", "COMBO")
zscoresmat[which(is.na(zscoresmat))] = 0
zscoresmat[which(abs(zscoresmat)>1000)] = 0
